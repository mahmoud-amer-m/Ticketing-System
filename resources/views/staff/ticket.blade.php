    ...
    <div class="row">
        <div class="col-xs-12 col-md-12">
            <div class="panel panel-default">
                <!--<div class="panel-heading"><a href="<?php // echo URL::action('HomeController@index', 0); ?>">الطلبات</a></div>-->
                <div class="no-print emp_segment" id="segment">
                    <?php
//                    var_dump($ticket_details);die();
                    if ($ticket_details->status != 4) {
                        ?> 
                        <a class="no-print" href="{{ URL::action('HomeController@closeticket', $ticket_details->ticket_id) }}">إغلاق</a> |
                        <?php
                    }
                    ?>
                    <a class="no-print action_only" data-title="<?php echo $ticket_details->title; ?>" data-tid="<?php echo $ticket_details->ticket_id; ?>" data-redirect="<?php // echo get_template_directory_uri() . '-child/staff-tickets-list.php?segment=5'  ?>" data-href="<?php // echo get_template_directory_uri() . '-child/staff-delete-ticket.php?ticketID=' . $ticket_details->ticket_id;  ?>" href="<?php // echo get_template_directory_uri() . '-child/staff-delete-ticket.php?ticketID=' . $ticket_details->ticket_id;  ?>">حذف</a> 
                    <?php
                    ?> 
                    | <a href="{{ URL::action('HomeController@edit', $ticket_details->ticket_id) }}">تعديل</a>
